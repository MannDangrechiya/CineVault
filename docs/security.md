...
- **Algorithm Enforcement**: Rejects `alg=none` and symmetric mock signatures in staging/production environments. Cryptographic RS256 validation executes against Keycloak JWKS endpoints.
- **Token Claims Verification**: Issuer (`iss`), Audience (`aud`), Expiration (`exp`), and Subject (`sub`) strictly verified before granting request context.
...
- **Machine Service Isolation**: Enforces zero-trust isolation boundaries across internal services:
  - Ingestion / AI Proposal services are strictly prohibited from direct write mutations to the `canonical` schema.
...
- `apps/web/src/app/api/proxy/[...path]/route.ts` intercepts all state-changing HTTP methods (`POST`, `PUT`, `PATCH`, `DELETE`).
...
- **Security Headers**: Standard production headers enforced on all API responses by `services/api/main.py`'s `SecurityHeadersMiddleware` (`X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Strict-Transport-Security: max-age=31536000; includeSubDomains`, `Content-Security-Policy: default-src 'self'`). As of the R1 infra fix, `infra/docker/Caddyfile` also sets HSTS on the web frontend's own HTTPS site — it does not yet set a CSP there (tracked as follow-up work; needs checking against the Next.js build's actual script/image sources first).
- **CAT-6 AI Governance**: AI proposal metadata is staged in `quality.ai_proposal_staging` with status `PENDING` and immutable SHA-256 HMAC audit log entries, requiring human curator review before canonical promotion.
