...
### Fixed
- **Catalog list endpoint mismatch** (`apps/web/src/lib/api/titles.ts`): `getCatalogPage()` and `getGenres()` called `/v1/titles` and `/genres`, but FastAPI mounts the offset-paginated `CatalogPageResponse` contract (`items`/`total`/`next_offset`, with query/genre/year/sort filters) at `/v1/catalog` and `/v1/genres`. `/v1/titles` is a separate, narrower cursor-paginated endpoint (`{data, pagination}`) with no filter support. The mismatch 404'd every catalog fetch, so `/movies`, `/series`, and `/search` always rendered empty ("The catalog is empty") despite 89,281 real titles in Postgres. This was also the actual cause behind the reported `/dashboard` production-smoke timeout — fixing the endpoint restored a clean `networkidle` on every page in `test_w13_production_smoke.js`. Root cause, not the low artwork-coverage number: confirmed by direct route probing against FastAPI (`/v1/catalog` → 200 with real rows, `/v1/titles` → different contract, `/titles` and `/genres` bare paths → 404).
- **Media isolation E2E false positive** (`apps/web/e2e/test_media_image_rendering.js`): the wrong-poster isolation test hardcoded `MOV-000007` as "Inception" — that `display_id` does not exist in the catalog (the real Inception record is `MOV-000002`), so the API correctly 404'd and `.poster_url` read as JS `undefined`. Traced PostgreSQL → repository → FastAPI → Next.js proxy → browser end-to-end for both `MOV-000001` (Parasite) and `MOV-000002` (Inception): both resolve distinct, real `image.tmdb.org` poster/backdrop URLs at every layer. There was no poster cross-contamination in the pipeline; the app's `string | null` media contract (`apps/web/src/lib/media.ts`, `services/api/media_resolver.py`) was already correct and untouched. Fixed the test fixture's ID.
- **Stale test assertions vs. intentionally sanitized W13 contracts**: `tests/test_observability_operations.py` asserted a `dependencies` key on `/health/readiness` that the W13 readiness-probe sanitization (see in-code comment in `services/api/routers/health.py`) deliberately renamed to `checks` to avoid exposing internal topology; `apps/web/tests/security.spec.ts` asserted a bare-string CSRF error body that the BFF's structured `{error: {code, message}}` envelope superseded. Verified the actual security/observability behavior directly (403 + `"CSRF verification failed: ..."` on cross-origin state-changing requests; `checks: {database, cache, queue}` on readiness) before updating the assertions — no security or observability regression, only stale expectations.
