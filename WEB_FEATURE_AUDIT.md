# Web App Feature Audit — 2026-08-25 / 2026-08-26 / 2026-08-30 / 2026-08-31 (x2)

**2026-08-31 W15 Media Isolation + Catalog Endpoint Repair Session:**
Reproduced and root-caused both reported regressions before touching code — neither was a media-pipeline bug:
- **Media isolation "failure"** was a test-fixture bug: `test_media_image_rendering.js` asked the API for `MOV-000007` as "Inception," a `display_id` that doesn't exist (the real Inception row is `MOV-000002`). The 404 response naturally had no `poster_url` key, so `.poster_url` read as `undefined`. Traced PostgreSQL → repository → FastAPI → Next.js proxy → browser for the correct IDs — Parasite and Inception each resolve distinct real `image.tmdb.org` URLs at every layer, no cross-contamination. Fixed the test's ID; 14/14 media E2E assertions now pass.
- **`/dashboard` production-smoke timeout** traced to `getCatalogPage()`/`getGenres()` calling `/v1/titles`/`/genres` — routes that either 404 or return an incompatible cursor-paginated shape — instead of the actual offset-paginated `/v1/catalog`/`/v1/genres` endpoints. This broke every catalog list page (movies/series/search render "catalog is empty" or crash on the shape mismatch), which is what was actually starving `networkidle` on `/dashboard`, not artwork coverage or a long-lived connection (confirmed: no polling/WebSocket/SSE on the dashboard route). Fixed the endpoint; `test_w13_production_smoke.js` now passes 15/15 twice in a row with the test's original 20s timeout untouched.
- **Artwork coverage is still 12/89,281 titles (0.01%)** — confirmed against the live database, not fabricated or backfilled. The 89k catalog is overwhelmingly synthetic Day 7/Phase 2 large-scale-ingestion fixture rows that were never run through a real artwork-enrichment pass. No stock/placeholder images were substituted; missing artwork renders an honest SVG placeholder.
- **Known, documented, not fixed (out of scope for this pass):** `test_stage_1000_performance_and_metrics`, `test_stage_5000_large_scale_distribution`, and `test_provenance_and_audit_retention` hang at the current catalog scale — root-caused via `pg_locks` to GIN-index (`idx_title_canonical_trgm`/`idx_title_original_trgm`) SELECT/INSERT lock contention inside the ingestion pipeline's per-item identity-match "large-catalog fallback" path (`services/api/ingestion/pipeline.py`). Pre-existing Day 7/Phase 2 architecture, unrelated to W14/W15 media work; fixing it means redesigning the ingestion transaction/locking strategy. See `CHANGELOG.md` [1.0.0-rc15] for full detail.
- Also fixed two stale test assertions (`tests/test_observability_operations.py`, `apps/web/tests/security.spec.ts`) that predated W13's intentional response-envelope sanitization — verified the real behavior (readiness `checks`, CSRF 403 + structured error) was already correct before updating the assertions.
